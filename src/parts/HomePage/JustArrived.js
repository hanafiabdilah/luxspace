...
function Loading() {
    return Array(6).fill().map((_, index) => {
        return(
            <div className="px-4 realative card group" key={index}>
    ...
    <section className="flex flex-col py-16">
        ...
        <div className="overflow-x-hidden px-4" id="carousel">
            <div className="container mx-auto" ref={refContainer}></div>
                {isLoading ? <div className="flex -mx-4 flex-row relative" style={{ paddingLeft:  refContainer.current?.getBoundingClientRect?.()?.left - 16 || 0}}> <Loading /> </div> : error ? JSON.stringify(error) : data.data.length === 0 ? "No Product Found" 
